...
async function AdMobShowBanner(test)
{
...
async function AdMobPrepareInterstitial(test)
{
...
async function AdMobPrepareRewardInterstitialAd(test)
{
...
async function AdMobPrepareRewardVideoAd(test)
{
...
async function PushNotificationsInit()
{
  ...
  window.addEventListener('fcmToken', function(e) {
    ...
    window.__fcmToken = e.detail.token.value;    
  });
...
async function enviarPush(type,delay = 0) {
  ...
  console.log(">#C04#> enviarPush. type: " + type + " token: " + token + " delay: " + delay)
  try {
