angular.module('starter').controller('irregverbsCtrl', function($scope, $rootScope, $state, $ionicSideMenuDelegate, $location, $ionicLoading, backendService ,$ionicSlideBoxDelegate ,$ionicScrollDelegate ,$ionicPosition, adsService, $ionicHistory, $stateParams, genService) {
...
$scope.cacheKey="irregverbs";

  genService.logEvent("IrregVerbs");

  adsService.showInterstitial(AdMob);

  $scope.$on( "$ionicView.beforeEnter", function( scopes, states ) 
  {
    $rootScope.viewTitle=$rootScope.i18n[$rootScope.loc]["irrv_title"]; 
  ...
  $scope.regverbs =[];
  console.log("* antes de getIrregVerbs *");
  $ionicLoading.show();
  ...
  backendService.doGetCached($scope.cacheKey,'/v3/irregverbs',$scope.userInfo,function(result) {
    console.log("* dentro de getIrregVerbs *");
    $ionicLoading.hide();
    console.log('* getIrregVerbs returned value *');
    console.log(result);
    //console.log(result.courses);
    // result.error contiene el mensaje de error si lo hay
    $scope.test="";
    $scope.irregverbs=result.data.irregverbs;
    ...
    var n=result.data.irregverbs.letras.length;
    for (i=0;i<n;i++)
    {
      $scope.paginas.push([result.data.irregverbs.letras[i].letra,result.data.irregverbs.letras[i].letra]);
    }
    //console.log($scope.paginas);

  });
  console.log("* después de getIrregVerbs *");
